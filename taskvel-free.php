...
<head>
    ...
    <meta name="description"
        content="Free browser-based task tracker and to-do list organizer. No sign-up, no account. Smart priority ranking, tags, deadlines, and a built-in Pomodoro focus timer. Works offline." />
    <link rel="canonical" href="https://www.samalconsultancy.com/taskvel-free.php" />
    <meta name="robots" content="index, follow, max-image-preview:large" />
    <meta property="og:type" content="website" />
    <meta property="og:site_name" content="Taskvel" />
    <meta property="og:title" content="Taskvel Free — Task Tracker & Pomodoro Timer" />
    <meta property="og:description"
        content="Organize tasks, auto-rank priorities, and track focus time. Free, no account required." />
    <meta property="og:url" content="https://www.samalconsultancy.com/taskvel-free.php" />
    <meta property="og:image" content="https://www.samalconsultancy.com/images/taskvel-og.png" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="Taskvel Free — Task Tracker & Pomodoro Timer" />
    <meta name="twitter:description"
        content="Free task tracker with smart priority ranking and a built-in focus timer." />
    <title>Taskvel Free · कार्य, done well. — Task Tracker & To-Do List with Pomodoro Timer</title>
    <link rel="icon"
    ...
    * {
    ...
    body.focus-mode .tagrow,
    body.focus-mode .tabs,
    body.focus-mode .list,
    body.focus-mode .remarks-view,
    body.focus-mode .time-report-view,
    body.focus-mode .matrix-view,
    body.focus-mode .review-view,
    ...
    <script type="application/ld+json">
    {
        ...
        "description": "Free browser-based task tracker with smart quick-add, Pomodoro focus timer, priority matrix, tags, deadlines, and offline PWA support. No account required.",
        "offers": {
            "@type": "Offer",
            "price": "0",
            "priceCurrency": "USD"
...
<body>
    ...
    <div class="sheet" id="sheet">
        ...
        <div class="fg"><label>What's the task?</label><input type="text" id="f-name"
                placeholder="e.g. Ship the Wispr auth endpoint" />
        ...
        <div class="fg"><label>Step 1</label>
            <div class="estep"><input type="text" id="f-s1" placeholder="First small move" /><input type="url"
                    id="f-sl1" placeholder="Link (optional)" /></div>
        </div>
        <div class="fg"><label>Step 2</label>
            <div class="estep"><input type="text" id="f-s2" placeholder="Optional" /><input type="url" id="f-sl2"
                    placeholder="Link (optional)" /></div>
        </div>
        <div class="fg"><label>Step 3</label>
            <div class="estep"><input type="text" id="f-s3" placeholder="Optional" /><input type="url" id="f-sl3"
                    placeholder="Link (optional)" /></div>
        </div>
    ...
    async function aiFocusBriefing() {
        ...
        try {
            const res = await fetch('api/ai_focus_public.php?action=focus', {
                method: 'POST',
            ...
            text.textContent = data.focus;
            line.style.display = 'block';
        } catch (e) {
            // Silent failure — the rule-based briefing already covers the
            // essentials; AI is a bonus, not a requirement.
        } finally {
    ...
    function resetAddForm() {
        ['f-name', 'f-person', 'f-collab', 'f-s1', 'f-s2', 'f-s3', 'f-sl1', 'f-sl2', 'f-sl3', 'f-deadline',
            'f-tag-input'
    ...
    // ════════════════════════════════════════════
    // AI SUGGEST — fills urgency/impact/deadline/steps/tags for whatever's
    // currently typed in the task-name field. This page has no login, so
    // it calls the public (IP rate-limited) AI endpoint, not the one used
    // by taskvel-pro.php.
    // ════════════════════════════════════════════
    async function aiSuggestTask() {
        ...
        try {
            const res = await fetch('api/ai_suggest_public.php?action=suggest', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, note: document.getElementById('f-person').value.trim() })
            });
            const data = await res.json().catch(() => ({}));
            if (!res.ok) throw new Error(data.error || `Request failed (${res.status})`);
            ...
            const deadlineEl = document.getElementById('f-deadline');
            if (deadlineEl && suggestion.deadline) deadlineEl.value = suggestion.deadline;
    ...
    function rmEStep(i) {
        ...
        document.getElementById('e-steps').querySelectorAll('.estep').forEach((row, n) => {
            ...
            if (t) t.id = 'est-' + n;
            const l = row.querySelector('input[type=url]');
            if (l) l.id = 'esl-' + n;
    ...
    function useTemplate(id) {
        ...
        setTimeout(() => {
            ...
            (tm.steps || []).forEach((s, i) => {
                ...
                const linp = document.getElementById('f-sl' + (i + 1));
                if (linp) linp.value = s.link || '';
            });
        }, 400);
    }
