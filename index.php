...
<head>
    ...
    <style>
        ...
        * {
        ...
        .mini-timer:hover { transform: translateY(-3px); box-shadow: 0 26px 60px -12px var(--accent-glow); }
        .mini-timer.dragging { transition: none; cursor: grabbing; }
        @keyframes miniTimerIn {
            from { opacity: 0; transform: translateY(16px) scale(.9); }
            to { opacity: 1; transform: none; }
        }
        .mini-ring { width: 40px; height: 40px; flex-shrink: 0; position: relative; }
        .mini-ring svg { transform: rotate(-90deg); }
        .mini-ring-bg { fill: none; stroke: var(--bg-sunk); stroke-width: 4; }
        .mini-ring-fg { fill: none; stroke: var(--accent); stroke-width: 4; stroke-linecap: round; transition: stroke-dashoffset .4s linear; }
        .mini-timer-body { display: flex; flex-direction: column; line-height: 1.25; min-width: 0; }
        .mini-timer-time { font-family: 'JetBrains Mono'; font-size: 15px; font-weight: 700; color: var(--ink); }
        .mini-timer-label { font-size: 9.5px; color: var(--ink3); text-transform: uppercase; letter-spacing: .8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 110px; }
        .mini-timer-btns { display: flex; gap: 6px; }
        .mini-timer-btns button {
            width: 28px; height: 28px; border-radius: 50%; border: 1px solid var(--line2);
            background: var(--bg); color: var(--ink2); cursor: pointer; font-size: 12px;
            display: flex; align-items: center; justify-content: center; transition: all .2s;
        }
        .mini-timer-btns button:hover { border-color: var(--accent); color: var(--accent); }
        @media (max-width: 380px) {
            .mini-timer { right: 12px; bottom: 12px; padding: 8px 12px 8px 8px; gap: 9px; }
            .mini-timer-label { max-width: 80px; }
        }
        ...
        .goal-bar:hover { border-color: var(--accent); box-shadow: var(--shadow); transform: translateY(-2px); }
        .goal-bar-top {
            display: flex;
            justify-content: space-between;
            align-items: center;
...
<body>
    ...
    <div class="app">
        <div class="header">
            <div class="brand-row">
                <div class="brand">
                    ...
                    <div class="brand-txt">
                        ...
                        <div class="tag">कार्य, Done Well</div>
                    </div>
                </div>
                <div class="head-right">
                    ...
                    <button class="icon-btn" onclick="location.href='teams.php'" aria-label="Teams & shared projects" title="Teams — assign & track tasks with coworkers">
                        <span class="tt-icon">👥</span>
                    </button>
                    <button class="icon-btn" onclick="location.href='checkin.php'" aria-label="Daily check-in" title="Daily Check-in — office mode: check in, report tasks, check out with a summary">
                        <span class="tt-icon">📍</span>
                    </button>
            ...
            <div class="panel" id="notif-panel">
                ...
                <button class="act" id="enable-push-btn" onclick="enablePushNotifications()" style="width:100%;justify-content:center;margin-bottom:10px">🔔 Enable push notifications on this device</button>
    ...
    <div class="sheet" id="sheet">
        ...
        <div class="fg"><label>What's the task?</label><input type="text" id="f-name" placeholder="e.g. Call client tomorrow #work !high" />
            <div style="font-size:10.5px;color:var(--ink4);margin-top:6px;font-family:'JetBrains Mono'">✨ Smart parse: #tags, !urgent/!high/!medium/!low, today/tomorrow/next monday/in 3 days</div>
        </div>
    ...
    <div class="sheet" id="e-sheet">
        ...
        <div class="fg"><label>Links / resources</label>
            <div class="tag-input-row"><input type="text" id="e-link-label" placeholder="Label (e.g. Design doc)" style="flex:1" /></div>
            <div class="tag-input-row"><input type="url" id="e-link-url" placeholder="https://…" style="flex:1" /><button class="tag-add-btn" onclick="addLinkToEditForm()">Add</button></div>
    ...
    <div class="sheet" id="ct-sheet" style="max-width:380px">
        ...
        <div class="fg"><label>Focus minutes</label><input type="number" id="ct-focus" min="1" max="180" value="25" /></div>
        <div class="fg"><label>Break minutes</label><input type="number" id="ct-break" min="1" max="60" value="5" /></div>
    ...
    <div class="mini-timer" id="mini-timer" onclick="miniTimerClick(event)">
        <div class="mini-ring">
            <svg width="40" height="40" viewBox="0 0 40 40">
                ...
                <circle class="mini-ring-fg" id="mini-ring" cx="20" cy="20" r="17" stroke-dasharray="106.8" stroke-dashoffset="0" />
            </svg>
        </div>
    ...
    <script>
        ...
        function load() {
            ...
            try { dailyGoal = parseInt(localStorage.getItem(LS_GOAL)) || 5; } catch (e) { dailyGoal = 5; }
        }
        ...
        async function pullStateFromServer() {
            try {
                ...
                if (state.theme) { document.documentElement.setAttribute('data-theme', state.theme); localStorage.setItem(LS_THEME, state.theme); }
                if (state.accent) { document.documentElement.setAttribute('data-accent', state.accent); localStorage.setItem(LS_ACCENT, state.accent); }
                tasks.forEach(t => {
                    if (!Array.isArray(t.tags)) t.tags = [];
                    if (!t.recur) t.recur = 'none';
                    if (typeof t.collab !== 'string') t.collab = '';
        ...
        function setDailyGoal() {
            ...
            if (!n || n < 1) { toast('Enter a number of at least 1'); return; }
            dailyGoal = n;
            try { localStorage.setItem(LS_GOAL, String(n)); } catch (e) {}
            schedulePush();
            renderGoalBar();
        ...
        function checkDailyGoal() {
            const today = todayKey();
            if (localStorage.getItem(LS_GOAL_CELEBRATED) === today) { renderGoalBar(); return; }
            if (tasksCompletedToday() >= (dailyGoal || 5)) {
                localStorage.setItem(LS_GOAL_CELEBRATED, today);
                showCelebration('🎯', 'Daily goal reached!', `You've completed ${dailyGoal} task${dailyGoal === 1 ? '' : 's'} today — that's a wrap.`, 4000);
            }
            renderGoalBar();
        }
        ...
        function addLinkToEditForm() {
            ...
            editLinks.push({ id: Date.now(), label, url });
            labelInp.value = ''; urlInp.value = '';
            renderEditLinks();
        }
        function renderEditLinks() {
            document.getElementById('e-links').innerHTML = editLinks.map((l, i) => `<span class="tagpill-x">🔗 ${esc(l.label)}<button onclick="removeEditLink(${i})" aria-label="Remove link">✕</button></span>`).join('');
        }
        ...
        function exportICS() {
            ...
            if (!withDeadline.length) { toast('No tasks with deadlines to export'); return; }
            ...
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = `taskvel-calendar-${todayKey()}.ics`;
    ...
    function restoreData(e) {
        ...
        function cardHTML(t, idx) {
            ...
            const linksHtml = (t.links || []).length ? `<div class="tag-row">${t.links.map(l => `<a class="tagpill-x" style="text-decoration:none" href="${esc(l.url)}" target="_blank" rel="noopener">🔗 ${esc(l.label || l.url)}</a>`).join('')}</div>` : '';
    ...
    if (t && t.timeTrackingStarted) {
        ...
        toast('Logged ' + formatTime(duration) + ' ⏱');
                        ...
                        function smartParseTask(raw) {
                            let text = raw;
                            const tags = [];
                            text = text.replace(/#([\w-]+)/g, (m, tag) => { tags.push(normalizeTag(tag)); return ''; });
                            ...
                            for (const [re, val] of urgencyMap) {
                                if (re.test(text)) { urgency = val; text = text.replace(re, ''); break; }
                            }

                            let deadline = null;
                            const today = new Date(); today.setHours(0, 0, 0, 0);
                            if (/\btoday\b/i.test(text)) {
                                deadline = todayKey(today); text = text.replace(/\btoday\b/i, '');
                            } else if (/\btomorrow\b/i.test(text)) {
                                const d = new Date(today); d.setDate(d.getDate() + 1);
                                deadline = todayKey(d); text = text.replace(/\btomorrow\b/i, '');
                            } else {
                                ...
                                if (inDays) {
                                    ...
                                    deadline = todayKey(d); text = text.replace(inDays[0], '');
                                } else if (wdMatch) {
                                    ...
                                    deadline = todayKey(d); text = text.replace(wdMatch[0], '');
                                }
                            }

                            text = text.replace(/\s{2,}/g, ' ').trim();
                            return { name: text, tags, deadline, urgency };
                        }
                        function addTask() {
                            const rawName = document.getElementById('f-name').value.trim();
                            if (!rawName) { toast('Enter the task name first'); return }
                            const parsed = smartParseTask(rawName);
                            parsed.tags.forEach(tg => { if (!formTags.includes(tg)) formTags.push(tg); });
                            if (!sel.urg && parsed.urgency) sel.urg = parsed.urgency;
                            if (!sel.dmg) sel.dmg = ({ critical: 'severe', high: 'moderate', medium: 'moderate', low: 'minor' }[sel.urg] || 'moderate');
                            if (!document.getElementById('f-deadline').value && parsed.deadline) document.getElementById('f-deadline').value = parsed.deadline;
                        ...
                        function saveTemplates() { localStorage.setItem(LS_TEMPLATES, JSON.stringify(templates)); schedulePush(); }
                        ...
                        function loadCustomTimerMode() {
                            try {
                                ...
                                if (saved && saved.f && saved.b) return { f: saved.f, b: saved.b, label: 'Custom' };
                            } catch (e) {}
                            return { f: 20, b: 5, label: 'Custom' };
                        }
                        const MODES = [{ f: 25, b: 5, label: '25 / 5' }, { f: 50, b: 10, label: '50 / 10' }, { f: 15, b: 3, label: '15 / 3' }, loadCustomTimerMode()];
                        ...
                        function cycleMode() {
                            ...
                            if (MODES[modeIdx].label === 'Custom') { openCustomTimer(); return; }
                            resetTimer(); toast('Timer set to ' + MODES[modeIdx].label);
                        }
                        function openCustomTimer() {
                            ...
                            document.getElementById('ct-ov').classList.add('open'); document.getElementById('ct-sheet').classList.add('open');
                        }
                        function closeCustomTimer() {
                            document.getElementById('ct-ov').classList.remove('open'); document.getElementById('ct-sheet').classList.remove('open');
                        }
                        function saveCustomTimer() {
                            ...
                            MODES[MODES.length - 1] = { f, b, label: 'Custom' };
                            try { localStorage.setItem(LS_CUSTOM_TIMER, JSON.stringify({ f, b })); } catch (e) {}
                        ...
                        function miniTimerClick(e) {
                            ...
                            document.querySelector('.focus').scrollIntoView({ behavior: 'smooth', block: 'center' });
                        }
                        function miniToggle(e) { e.stopPropagation(); toggleTimer(); }
                        function miniStop(e) {
                            e.stopPropagation();
                            resetTimer();
                            focusTaskId = null;
                        ...
                        function fireConfetti() {
                            ...
                            function frame(now) {
                                ...
                                let alive = false;
                                pieces.forEach(p => {
                                    p.vy += gravity; p.x += p.vx; p.y += p.vy; p.rot += p.vr; p.life = now - start;
                        ...
                        async function enablePushNotifications() {
                            ...
                            try {
                                if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
                                    toast('Push notifications aren\'t supported in this browser'); return;
                                }
                                const perm = await Notification.requestPermission();
                                if (perm !== 'granted') { toast('Notification permission was not granted'); return; }
                                const { key } = await Taskvel.settings.vapidPublicKey();
                                if (!key) { toast('Push isn\'t configured on the server yet'); return; }
                                ...
                                if (btn) { btn.textContent = '✅ Push notifications enabled'; btn.disabled = true; }
                        ...
                        async function refreshPushButtonState() {
                            ...
                            try {
                                ...
                                if (sub && Notification.permission === 'granted') { btn.textContent = '✅ Push notifications enabled'; btn.disabled = true; }
                            } catch (e) {}
                        }
                        ...
                        document.addEventListener('keydown', e => {
                            if (e.key === 'Escape') {
                                closeSheet(); closeEdit(); closeRemark(); closeAllPanels(); dismissCelebration(); closeCmdk(); closeCustomTimer();
                                if (document.activeElement && document.activeElement.blur) document.activeElement.blur();
                                return;
                            }
                            const el = e.target;
                            ...
                            else if (e.key === '?') { e.preventDefault(); toast('⌘K commands · N new · / search · Space timer · T dark · D done · Esc close', null, null, 4200); }
                        });
                        ...
                        const CMDK_ACTIONS = [
                            ...
                            { label: 'Set custom timer', icon: '⚙', run: () => openCustomTimer() },
                            { label: 'Export as CSV', icon: '▦', run: () => exportCSV() },
                            { label: 'Export as PDF', icon: '▥', run: () => exportPDF() },
                            { label: 'Export as Calendar (.ics)', icon: '📅', run: () => exportICS() },
                            { label: 'Set daily goal', icon: '🎯', run: () => setDailyGoal() },
                            { label: 'Teams & shared projects', icon: '👥', run: () => location.href = 'teams.php' },
                            { label: 'Daily Check-in (office mode)', icon: '📍', run: () => location.href = 'checkin.php' },
                            { label: 'Manager Dashboard', icon: '📊', run: () => location.href = 'manager.php' },
